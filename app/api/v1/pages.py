...
@router.get(
    ...
    summary="Get embedded page image",
    description="""
Retrieve an embedded image from a PDF page by its cross-reference (xref) number.

PDF documents contain embedded images that are referenced by unique xref numbers. This endpoint extracts and returns the raw image data in its original format (PNG or JPEG).

Use cases:
- Extracting images from PDF documents for reuse
- Displaying embedded images in a document viewer
- Downloading specific images from a PDF
...
""",
    responses={
        ...
        404: {"description": "Document, page, or image not found"},
        401: {"description": "Authentication required"},
    },
    openapi_extra={
        "x-codeSamples": [
            {
                "lang": "curl",
                "label": "cURL",
                "source": """curl -X GET "https://api.giga-pdf.com/api/v1/documents/{document_id}/pages/1/images/7" \\
  -H "Authorization: Bearer $TOKEN" \\
...
response = requests.get(
    f"https://api.giga-pdf.com/api/v1/documents/{document_id}/pages/1/images/{image_xref}",
    headers={"Authorization": f"Bearer {token}"}
)
...
print(f"Image type: {content_type}")""",
            },
            {
                "lang": "javascript",
                "label": "JavaScript",
                "source": """// Display in an img element
const img = document.getElementById('pdf-image');
img.src = `https://api.giga-pdf.com/api/v1/documents/${documentId}/pages/1/images/${imageXref}`;

// Or fetch and create a blob URL
const response = await fetch(
  `https://api.giga-pdf.com/api/v1/documents/${documentId}/pages/1/images/${imageXref}`,
  { headers: { "Authorization": `Bearer ${token}` } }
);
...
const imageUrl = URL.createObjectURL(blob);""",
            },
            {
                ...
                "label": "PHP",
                "source": """<?php
$ch = curl_init("https://api.giga-pdf.com/api/v1/documents/$document_id/pages/1/images/$image_xref");
curl_setopt($ch, CURLOPT_HTTPHEADER, ["Authorization: Bearer $token"]);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
// Save to file
file_put_contents("extracted_image.png", $image_data);

// Or output directly with proper headers
